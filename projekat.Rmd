...
sample_df <- sdf_sample(df, 0.3, TRUE, 123)
```

```{r vr-Bottles_Sold, eval=FALSE, include=FALSE }
grouped_df <- sample_df2 %>%
...
```{r decision-tree-classifier, echo=FALSE, eval=FALSE, include=TRUE}
training_cols <- colnames(clusterisation_dataset$training)
validation_df <- clusterisation_dataset$validation %>%
  select(all_of(training_cols))

dtc_model <- clusterisation_dataset$training %>%
  ml_decision_tree_classifier(Pack ~ Bottle_Volume_ml + State_Bottle_Cost)

dtc_accuracy <- ml_evaluate(dtc_model, validation_df)
```